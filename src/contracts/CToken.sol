 ...
 * @author Compound
 */
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter, BlastCommon {
    /**
    ...
    function accrueInterest() public virtual override returns (uint256) {
        ...
        uint256 totalBorrowsNewBeforeRebase = (simpleInterestFactor * borrowsPrior) / 1e18;
        uint256 interestAccumulated = totalBorrowsNewBeforeRebase - borrowsPrior;
        uint256 totalReservesNew =
            (reservesPrior * rebaseFactor) / lastRebaseFactor + ((reserveFactorMantissa * interestAccumulated) / 1e18);

        uint256 totalBorrowsNew = (totalBorrowsNewBeforeRebase * rebaseFactor) / lastRebaseFactor;
        uint256 borrowIndexNew = (((simpleInterestFactor * borrowIndexPrior) / 1e18) * rebaseFactor) / lastRebaseFactor;
