...
import "src/lib/openzeppelin-contracts/contracts/token/ERC20/IERC20.sol";
import "./CToken.sol";
 ...
 * @author Compound
 */
contract Comptroller is ComptrollerV7Storage, ComptrollerErrorReporter, ExponentialNoError, BlastCommon {
     ...
     * @return The list of market addresses
     */
    function getEnteredMarkets(address addr) public view returns (CToken[] memory) {
     ...
     */
    event Redeem(address cToken, address redeemer, uint256 redeemAmount, uint256 redeemTokens);

    function emitRedeem(address redeemer, uint256 redeemAmount, uint256 redeemTokens) external {
    ...
    function emitBorrow(
        ...
        require(markets[msg.sender].isListed, "unlisted");
        emit Borrow(msg.sender, borrower, borrowAmount, accountBorrows, totalBorrows, accountStaticBorrows);
    }

    event Mint(address indexed cToken, address minter, uint256 mintAmount, uint256 mintTokens);

    function emitMint(address minter, uint256 mintAmount, uint256 mintTokens) external {
        require(markets[msg.sender].isListed, "unlisted");
    ...
    function emitRepayBorrow(
        ...
        require(markets[msg.sender].isListed, "unlisted");
        emit RepayBorrow(msg.sender, payer, borrower, repayAmount, accountBorrows, totalBorrows, accountStaticBorrows);
    }
    ...
    function emitLiquidateBorrow(
        ...
        require(markets[msg.sender].isListed, "unlisted");
        emit LiquidateBorrow(msg.sender, liquidator, borrower, repayAmount, cTokenCollateral, seizeTokens);
    }

    event AccrueInterest(
        address indexed cToken, uint256 interestAccumulated, uint256 borrowIndex, uint256 exchangeRate
    );
    ...
    event Transferred(address indexed cToken, address src, address dst, uint256 srcBalance, uint256 dstBalance);

    function emitTransfer(address src, address dst, uint256 srcBalance, uint256 dstBalance) external {
        require(markets[msg.sender].isListed, "unlisted");
